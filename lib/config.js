...
import {DISCOVERY_OPTIONS} from './discovery.js';
 ...
 * @param {boolean} [input.discovery] the adapter has a discovery hint: adds --discover,
 *        --discover-json and --discover-timeout (handled by runDiscovery, see lib/discovery.js)
...
export function parseConfig({
    ...
    for (const [key, def] of Object.entries(merged)) {
        ...
        // `secret` and `file` are schema metadata only, not yargs option properties
        // eslint-disable-next-line no-unused-vars
        const {secret, file, ...yargsDef} = def;
        let withEnv =
