...
import {DISCOVERY_OPTIONS, discoveryKinds} from './discovery.js';
...
function allOptions({options = {}, defaults = {}, discovery = false}) {
    const merged = {...options, ...(discovery ? DISCOVERY_OPTIONS : {})};
 ...
 * `'network'`, `'serial'`, or both. A schema with an `x-discover` property is a
 * discovery-capable adapter; there is no separate flag (she I13).
 *
 ...
 */
export function configSchema({pkg, envPrefix, options, defaults, scriptName, discovery}) {
    const properties = {};
    const required = [];
...
export function parseConfig({
    ...
    const merged = allOptions({options, defaults, discovery});
    if (argv.includes('--config-schema')) {
        // before yargs, so required options do not get in the way
        print(JSON.stringify(configSchema({pkg, envPrefix, options, defaults, scriptName, discovery}), null, 2));
        exit(0);
        return {};
    }
    ...
    for (const [key, def] of Object.entries(merged)) {
        ...
        if (uninstalling && withEnv.demandOption) {
