...
/** Unprefixed broker variables read as fallback when <ENVPREFIX>_<X> is absent (B-3). */
export const SHARED_ENV = ['MQTT_URL', 'MQTT_USERNAME', 'MQTT_PASSWORD', 'MQTT_CLIENT_ID_PREFIX', 'MQTT_TLS_CA'];
...
export const SHARED_OPTIONS = {
    ...
    'mqtt-password': {type: 'string', describe: 'mqtt broker password'},
 ...
 * the kebab-case option name, with `x-env` carrying the environment variable name.
 */
export function configSchema({pkg, envPrefix, options, defaults, scriptName}) {
    ...
    const schema = {
        ...
        'x-adapter': {name: pkg.name, version: pkg.version, envPrefix},
    };
...
export function parseConfig({
    ...
    for (const [key, def] of Object.entries(merged)) {
        ...
        const withEnv = fromEnv === undefined ? def : {...def, default: coerce(fromEnv, def.type), demandOption: false};
