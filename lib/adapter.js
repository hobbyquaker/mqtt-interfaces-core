 ...
 *   <name>/status/<item>            retained; plain or {val, ts, lc} (--json-payloads)
 ...
 * @param {(ctx: {get: Function, config: object}) => {id?: string, device?: object, components: object, availabilityMin?: number} | null} [a.discovery]
 *        builds the HA device block + entity map from the last known status values
...
export function createAdapter({
    ...
    function buildDiscovery() {
        ...
        const {id, device, components, availabilityMin} = built;
    ...
    function publishDiscovery({force = false} = {}) {
        ...
        if (lastDiscoveryId && lastDiscoveryId !== built.topic) {
            publish(lastDiscoveryId, '', {retain: true});
        }
        lastDiscoveryId = built.topic;
        log.info('mqtt publishing home assistant discovery', built.topic);
        publish(built.topic, built.payload, {retain: true});
    }

    function clearDiscovery() {
        const built = buildDiscovery();
        if (built) {
            publish(built.topic, '', {retain: true});
        }
    }
