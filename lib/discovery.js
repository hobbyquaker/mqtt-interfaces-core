...
export async function mdnsQuery({
    ...
    try {
        for (let i = 0; i < tries; i++) {
            socket.send(query, 0, query.length, port, address);
            if (i + 1 < tries) {
                await delay(Math.min(300, timeout / (tries + 1)));
            }
        }
...
export async function discover(hint = {}, options = {}) {
    ...
    for (const spec of [].concat(hint.ssdp || [])) {
        tasks.push(
            ssdpSearch({...spec, timeout, createSocket: deps.createSocket})
                .then((answers) => {
    ...
    for (const spec of [].concat(hint.mdns || [])) {
        tasks.push(
            mdnsQuery({...spec, timeout, createSocket: deps.createSocket})
                .then((entries) => {
                    for (const entry of entries) {
    ...
    for (const spec of [].concat(hint.udp || [])) {
        tasks.push(
            udpProbe({...spec, timeout, createSocket: deps.createSocket})
                .then((answers) => {
